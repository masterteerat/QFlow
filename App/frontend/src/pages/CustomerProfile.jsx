...
export default function CustomerProfile() {
  ...
  useEffect(() => {
    if (!customer?.id) return navigate('/login');
    api.get(`/customer/profile/${customer.id}`).then((res) => {
      if (res.success && res.user) setForm({ fname: res.user.fname, lname: res.user.lname, phone_number: res.user.phone_number || '' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.put(`/customer/profile/${customer.id}`, form);
      if (res.success) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-300">
      <Navbar userName={form.fname || customer?.fname} profileLink="/customer-profile" />
